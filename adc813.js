    ...
    var AdCorrector = (function() {
        ...
        var ANALYSIS_VERSION = '2026.08.13';
        var REVIEW_SCHEMA_VERSION = '1.0';
        var currentArtworkMeta = {};
        var artworkFingerprintPromise = null;
...
ctx.drawImage(img, 0, 0, newWidth, newHeight);
        ...
        function getProjectDetails() {
            var fieldMap = {
                client: 'ac-projectClient',
                campaign: 'ac-projectCampaign',
                creative: 'ac-projectCreative',
                placement: 'ac-projectPlacement',
                reference: 'ac-projectReference',
                reviewer: 'ac-projectReviewer'
            };
            var details = {};
            Object.keys(fieldMap).forEach(function(key) {
                var field = document.getElementById(fieldMap[key]);
                details[key] = sanitizeText(field ? field.value.trim() : '');
            });
            return details;
        }
        ...
        function setProjectDetails(details) {
            ...
            Object.keys(fieldMap).forEach(function(key) {
                var field = document.getElementById(fieldMap[key]);
                if (field) field.value = sanitizeText(details[key] || '');
            });
        }

        function clearProjectDetails() {
            setProjectDetails({});
            var panel = document.getElementById('ac-workflowDetailsPanel');
            if (panel) panel.open = false;
        }

        function createReviewRecord() {
            var now = new Date();
            var compactDate = now.toISOString().slice(0, 10).replace(/-/g, '');
            var randomPart = Math.random().toString(36).slice(2, 8).toUpperCase();
            window.acReviewRecord = {
                id: 'ACR-' + compactDate + '-' + randomPart,
                reviewedAt: now.toISOString(),
                analysisVersion: ANALYSIS_VERSION,
                status: 'Directional review complete'
            };
            return window.acReviewRecord;
        }
        ...
        function blobToArrayBuffer(blob) {
            ...
            return new Promise(function(resolve, reject) {
                ...
                reader.onerror = function() { reject(reader.error || new Error('Could not read artwork bytes.')); };
        ...
        function bufferToHex(buffer) {
            ...
            for (var i = 0; i < bytes.length; i++) {
                ...
                output += byteHex.length === 1 ? '0' + byteHex : byteHex;
            }
            return output;
        }

        function prepareArtworkMetadata(image, sourceFile) {
            currentArtworkMeta = {
                sourceFileName: sanitizeText(sourceFile && sourceFile.name ? sourceFile.name : currentArtworkMeta.sourceFileName || 'Uploaded artwork'),
                sourceMimeType: sourceFile && sourceFile.type ? sourceFile.type : currentArtworkMeta.sourceMimeType || '',
                sourceFileSizeBytes: sourceFile && typeof sourceFile.size === 'number' ? sourceFile.size : Number(currentArtworkMeta.sourceFileSizeBytes || 0),
                analyzedWidthPx: image ? Number(image.width || 0) : Number(currentArtworkMeta.analyzedWidthPx || 0),
                analyzedHeightPx: image ? Number(image.height || 0) : Number(currentArtworkMeta.analyzedHeightPx || 0),
                fingerprintAlgorithm: 'SHA-256',
                fingerprint: currentArtworkMeta.fingerprint || ''
            };

            artworkFingerprintPromise = imageToBlob(image).then(function(blob) {
                currentArtworkMeta.analyzedMimeType = blob.type || currentArtworkMeta.sourceMimeType || 'image/unknown';
                currentArtworkMeta.analyzedFileSizeBytes = Number(blob.size || 0);
                if (!window.crypto || !window.crypto.subtle || typeof window.crypto.subtle.digest !== 'function') {
                    currentArtworkMeta.fingerprintAlgorithm = 'Unavailable';
                    currentArtworkMeta.fingerprint = '';
                    return currentArtworkMeta;
                }
                return blobToArrayBuffer(blob).then(function(buffer) {
                    return window.crypto.subtle.digest('SHA-256', buffer);
                }).then(function(hashBuffer) {
                    currentArtworkMeta.fingerprint = bufferToHex(hashBuffer);
                    return currentArtworkMeta;
                });
            }).catch(function(error) {
                console.warn('Artwork fingerprint unavailable:', error);
                currentArtworkMeta.fingerprintAlgorithm = 'Unavailable';
                currentArtworkMeta.fingerprint = '';
                return currentArtworkMeta;
            });

            return artworkFingerprintPromise;
        }

        function ensureArtworkMetadata() {
            if (currentArtworkMeta.fingerprint || currentArtworkMeta.fingerprintAlgorithm === 'Unavailable') {
                return Promise.resolve(currentArtworkMeta);
            }
            if (artworkFingerprintPromise) return artworkFingerprintPromise;
            if (!uploadedImage) return Promise.resolve(currentArtworkMeta);
            return prepareArtworkMetadata(uploadedImage, null);
        }

        function shortArtworkFingerprint() {
            return currentArtworkMeta.fingerprint ? currentArtworkMeta.fingerprint.slice(0, 16).toUpperCase() : 'Unavailable';
        }
        ...
        function applyScenarioControls(renderPreview) {
            ...
            if (renderPreview && uploadedImage) {
                var speedTab = document.getElementById('tab-speed');
                if (speedTab && !speedTab.classList.contains('active')) {
                    switchTab('speed');
                }
                renderSpeedView();
            }
  ...
  return;
    ...
    if (totalWords > 0) {
        ...
        function buildClientSummary() {
            ...
            var inputs = window.acLastInputs || getCurrentFormInputs();
            var project = getProjectDetails();
            var review = window.acReviewRecord || {};
            ...
            if (project.client) lines.push('Client / Advertiser: ' + project.client);
            if (project.campaign) lines.push('Campaign: ' + project.campaign);
            if (project.creative) lines.push('Creative / Version: ' + project.creative);
            if (project.placement) lines.push('Market / Placement: ' + project.placement);
            if (project.reference) lines.push('Media Unit / Reference: ' + project.reference);
            if (project.reviewer) lines.push('Reviewer: ' + project.reviewer);
            if (project.client || project.campaign || project.creative || project.placement || project.reference || project.reviewer) {
                lines.push('');
            }
        ...
        function bindWorkflowDetails() {
            var downloadButton = document.getElementById('ac-downloadReviewData');
            if (downloadButton) downloadButton.addEventListener('click', downloadReviewData);
        }
        ...
        function saveCurrentTest() {
            ...
            ensureArtworkMetadata().then(function() {
                return getSavedTests();
                ...
                var record = {
                    ...
                    imageBlob: imageBlob,
                    inputs: window.acLastInputs || getCurrentFormInputs(),
                    ...
                    artworkMeta: Object.assign({}, currentArtworkMeta),
                    reviewRecord: Object.assign({}, window.acReviewRecord || {}),
                    ocrGeometry: {
                        boxes: Array.isArray(analysisData.ocrBoxes) ? analysisData.ocrBoxes : [],
                        wordBoxes: Array.isArray(analysisData.ocrWordBoxes) ? analysisData.ocrWordBoxes : [],
                        lineCandidates: Array.isArray(analysisData.ocrLineCandidates) ? analysisData.ocrLineCandidates : []
                    }
                };
            ...
            getSavedTest(id).then(function(record) {
                ...
                restoredImage.onload = function() {
                    resizeImageIfNeeded(restoredImage).then(function(processedImage) {
                        ...
                        var savedOcrGeometry = record.ocrGeometry || {};
                        analysisData = {
                            ocrBoxes: Array.isArray(savedOcrGeometry.boxes) ? savedOcrGeometry.boxes : [],
                            ocrWordBoxes: Array.isArray(savedOcrGeometry.wordBoxes) ? savedOcrGeometry.wordBoxes : [],
                            ocrLineCandidates: Array.isArray(savedOcrGeometry.lineCandidates) ? savedOcrGeometry.lineCandidates : []
                        };
                        ...
                        window.acLastInputs = null;
                        currentArtworkMeta = record.artworkMeta ? Object.assign({}, record.artworkMeta) : {};
                        artworkFingerprintPromise = currentArtworkMeta.fingerprint
                            ? Promise.resolve(currentArtworkMeta)
                            : prepareArtworkMetadata(processedImage, {
                                name: currentArtworkMeta.sourceFileName || record.name || 'Saved artwork',
                                type: record.imageBlob.type || '',
                                size: record.imageBlob.size || 0
                            });
                        setProjectDetails(record.projectDetails || {});
                        window.acRestoredReviewRecord = record.reviewRecord && record.reviewRecord.id
                            ? Object.assign({}, record.reviewRecord)
                            : null;
                        if (typeof window.peReset === 'function') window.peReset();
        ...
        function handleFileUpload(file) {
            try {
                ...
                window.acLastInputs = null;
                currentArtworkMeta = {};
                artworkFingerprintPromise = null;
                window.acReviewRecord = null;
                window.acRestoredReviewRecord = null;
                if (typeof window.peReset === 'function') window.peReset();
                ...
                reader.onload = function(e) {
                    ...
                    uploadedImage.onload = function() {
                        resizeImageIfNeeded(uploadedImage).then(function(processedImg) {
                            ...
                            prepareArtworkMetadata(uploadedImage, file);
...
window.acLastInputs = getCurrentFormInputs();
if (window.acRestoredReviewRecord) {
  window.acReviewRecord = window.acRestoredReviewRecord;
  window.acRestoredReviewRecord = null;
} else {
  createReviewRecord();
}
...
window.acLastInputs = window.acLastInputs || null;
...
function acRecomputeFromLastRun() {
  ...
  if (window.acLastInputs) window.acLastInputs.mode = window.acScoringMode;
  createReviewRecord();
...
function renderGlare() {
        ...
        function downloadReport() {
            if (!window.acLastScores || !window.acLastDetails) {
                alert('Run an analysis before downloading a report.');
                announceToScreenReader('Run an analysis before downloading a report.');
                return;
            }
        ...
        function generateReportDownload() {
            try {
                ...
                var mode = (window.acScoringMode || 'direct');
                var project = getProjectDetails();
                var review = window.acReviewRecord || createReviewRecord();
                var reviewedInputs = window.acLastInputs || getCurrentFormInputs();
                ...
                ctx.fillText('Ad Corrector\u00AE Creative Review', 60, 90);
                
                ctx.font = '28px Arial';
                ctx.fillText('Directional OOH Visibility Analysis', 60, 135);
...
var fixesHeading = isATier ? 'Additional Recommendations' : 'Additional Observations';
		...
		recommendations.push('Use the attention heatmap and speed view to validate hierarchy and visibility.');
                ...
                ctx.fillStyle = '#333';
                ctx.font = 'bold 32px Arial';
                ctx.fillText('Review Record', 60, yPos);
                ...
                var reviewLines = [
                    ...
                    'Analysis version: ' + review.analysisVersion,
                    'Campaign goal: ' + (reviewedInputs.mode === 'brand' ? 'Brand Awareness' : 'Call to Action Campaign'),
                    'Reviewed conditions: ' + reviewedInputs.boardLabel + ' | ' + reviewedInputs.speed + ' mph | ' + reviewedInputs.distance + ' ft'
                ];

                if (project.client) reviewLines.push('Client / Advertiser: ' + project.client);
                if (project.campaign) reviewLines.push('Campaign: ' + project.campaign);
                if (project.creative) reviewLines.push('Creative / Version: ' + project.creative);
                if (project.placement) reviewLines.push('Market / Placement: ' + project.placement);
                if (project.reference) reviewLines.push('Media Unit / Reference: ' + project.reference);
                if (project.reviewer) reviewLines.push('Reviewer: ' + project.reviewer);
                if (currentArtworkMeta.sourceFileName) reviewLines.push('Artwork file: ' + currentArtworkMeta.sourceFileName);
                if (currentArtworkMeta.analyzedWidthPx && currentArtworkMeta.analyzedHeightPx) {
                    reviewLines.push('Analyzed artwork dimensions: ' + currentArtworkMeta.analyzedWidthPx + ' x ' + currentArtworkMeta.analyzedHeightPx + ' px');
                }
                reviewLines.push('Artwork fingerprint (SHA-256, first 16): ' + shortArtworkFingerprint());

                for (var reviewIndex = 0; reviewIndex < reviewLines.length; reviewIndex++) {
                    yPos = drawWrappedText(reviewLines[reviewIndex], 80, yPos, 1460, 30);
                    yPos += 4;
                }
                ...
                yPos = drawWrappedText('This record documents a directional preflight review. It is not advertiser, agency, production, media-owner, legal, accessibility, or platform approval.', 80, yPos, 1460, 28);
                ...
                var fileName = 'AdCorrector-Review-' + safeFileSegment(review.id || Date.now()) + '.png';
                        ...
                        if (isMobile && navigator.share && navigator.canShare && typeof File !== 'undefined') {
                            try {
                                ...
                                if (navigator.canShare({ files: [reportFile] })) {
                                    navigator.share({
                                        files: [reportFile],
                                        title: 'Ad Corrector\u00AE Review',
                                        text: 'Ad Corrector\u00AE directional review report'
                                    }).then(function() {
        ...
        function buildReviewDataObject() {
            var inputs = window.acLastInputs || getCurrentFormInputs();
            var details = window.acLastDetails || {};
            var review = window.acReviewRecord || createReviewRecord();
            var modeLabel = inputs.mode === 'brand' ? 'Brand Awareness' : 'Call to Action Campaign';
            ...
            return {
                schema: {
                    name: 'ad-corrector-creative-review',
                ...
                review: {
                    ...
                    status: review.status,
                    approval_status: 'Not an approval or certification'
                },
                project: getProjectDetails(),
                artwork: Object.assign({}, currentArtworkMeta),
                context: {
                    board_type: inputs.boardType,
                    board_label: inputs.boardLabel,
                    custom_height_ft: inputs.customHeight || null,
                    viewing_speed_mph: Number(inputs.speed) || 0,
                    viewing_distance_ft: Number(inputs.distance) || 0,
                    campaign_mode: inputs.mode,
                    campaign_mode_label: modeLabel,
                    entered_copy: {
                        headline: inputs.headline || '',
                        cta: inputs.mode === 'brand' ? '' : (inputs.cta || ''),
                        body: inputs.body || ''
                    },
                    word_count: Number(details.wordCount) || 0
                },
                results: {
                    grade: details.grade || 'N/A',
                    overall_score: Math.round(Number(details.avgScore) || 0),
                    metrics_percent: Object.assign({}, window.acLastScores || {})
                },
                contrast_accessibility_estimates: {
                    contrast_check: analysisData.adaCompliance || 'N/A',
                    estimated_ratio: analysisData.contrastRatioRaw ? analysisData.contrastRatioRaw + ':1' : 'N/A',
                    estimation_method: analysisData.contrastMethod || 'N/A',
                    local_samples: Number(analysisData.contrastLocalSamples) || 0,
                    color_blind_risk: analysisData.colorBlindRisk || 'N/A'
                },
                recommendations: {
                    fix_first: getListText('ac-actionPlanList', 3),
                    whats_working: getListText('ac-workingList', 3),
                    additional_observations: getListText('ac-fixesList', 3)
                },
                disclaimer: 'Directional preflight review based on uploaded artwork, selected viewing conditions, and user-entered copy. Not advertiser, agency, production, media-owner, legal, accessibility, platform, financial, or performance approval or certification.'
            };
        }

        function triggerDataDownload(blob, fileName) {
            var url = URL.createObjectURL(blob);
            var link = document.createElement('a');
            link.href = url;
            link.download = fileName;
        ...
        function downloadReviewData() {
            if (!window.acLastScores || !window.acLastDetails) {
                alert('Run an analysis before downloading review data.');
                announceToScreenReader('Run an analysis before downloading review data.');
                return;
            }

            ensureArtworkMetadata().then(function() {
                var exportObj = buildReviewDataObject();
                var jsonString = JSON.stringify(exportObj, null, 2);
                var fileName = 'AdCorrector-ReviewData-' + safeFileSegment(exportObj.review.id) + '.json';
                triggerDataDownload(new Blob([jsonString], { type: 'application/json;charset=utf-8' }), fileName);
                acTrackUsage('review_data_downloaded', { review_id: exportObj.review.id });
                announceToScreenReader('Review data download started.');
            }).catch(function(error) {
                console.error('Review data export error:', error);
                alert('Error exporting review data. Please try again.');
                announceToScreenReader('Error exporting review data.');
            });
        }
