...
function acApplyCampaignModeUI(mode) {
  ...
  if (bestResults) {
    bestResults.innerHTML = isBrand
      ? '<strong>For best results:</strong> Enter copy exactly as it appears on the ad. Headline and supporting text are evaluated separately.'
      : '<strong>For best results:</strong> Enter copy exactly as it appears on the ad. Headline, CTA, and body are evaluated separately.';
  }
...
if (label === 'Composition') {
  return acGetCompositionSummary(data, value);
}
