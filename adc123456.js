...
if (!globalLuminances.length) {
  ...
  throw new Error('Contrast estimate unavailable. No usable image samples were found.');
}
...
analysisData.adaCompliance = contrastCheck;
                ...
                throw new Error('Contrast estimate unavailable. Please try the original artwork file.');
            }
        }
  ...
  try {
    ...
    if (!mags.length) {
      throw new Error('No usable sharpness samples.');
    }
...
if (totalWords > wordBudget) {
  ...
  wordBudget: wordBudget
    ...
    throw new Error('Clarity estimate unavailable. Please try the original artwork file.');
  }
}
...
if (b > 255) b = 255;
    ...
    if (!totalSamples) throw new Error('No usable color samples.');
...
analysisData.colorBlindRisk = colorBlindRisk;
    ...
    throw new Error('Color estimate unavailable. Please try the original artwork file.');
  }
}
        ...
        function analyzeComposition(image) {
            try {
                ...
                throw new Error('Composition estimate unavailable. Please try the original artwork file.');
            }
        }
...
function acGetCompositionFixes(data, value) {
  if (Number(value) >= 85) {
    return ['Preserve the current spacing and visual hierarchy', 'Keep supporting elements from competing with the main message'];
  }
...
function displayGrade(gradeObj, score, data, details) {
  ...
  if (!hasCtaText && mode !== 'brand') {
    var missing = 'No CTA was entered. Enter the CTA as shown, or select Brand Awareness if no action is intended.';
  ...
  if (score >= 90) {
    msg = 'Strong estimated visibility under the selected conditions.';
    if (weakest.value < 90) {
      msg += ' Primary constraint: ' + weakest.label + ' (' + weakest.value + '%).';
    } else {
      msg += ' Preserve the current strengths of the artwork.';
    }
    // Context-aware verification distance
  }
  else if (score >= 80) {
    msg = 'Generally strong estimated visibility. Review the highlighted areas.';
    msg += ' Primary constraint: ' + weakest.label + ' (' + weakest.value + '%).';
  }
  else if (score >= 70) {
    msg = 'Some visibility concerns were identified. Review ' + weakest.label + '.';
    msg += ' Primary constraint: ' + weakest.label + ' (' + weakest.value + '%).';
  }
  else if (score >= 60) {
    msg = 'Several visibility concerns need attention. Focus on the priority recommendations.';
    msg += ' Primary constraint: ' + weakest.label + ' (' + weakest.value + '%).';
  }
  else {
    msg = 'Several substantial visibility concerns were identified. Review the main message and layout.';
    msg += ' Primary constraint: ' + weakest.label + ' (' + weakest.value + '%).';
  }
...
function acGetCtaCopyGuidance(text) {
  ...
  var hasEnteredCopy = !![headlineEl, ctaEl, document.getElementById('ac-bodyText')].some(function(el) {
    return el && !(el === ctaEl && window.acScoringMode === 'brand') && String(el.value || '').trim();
  });
  ...
  var metricDescriptions = {
    'Readability': 'Compares the copy you entered with format-specific word-count guidance.',
    ...
    'CTA': 'Reviews the length and action wording of the CTA you entered.'
  };
  ...
  function buildFixes(label, value) {
   ...
   if (label === 'Readability') {
    ...
    if (label === 'Contrast') {
      ...
      if (value >= 70) return ['Increase separation around the primary copy', 'Consider a plain background behind text placed on busy imagery'];
    ...
    if (label === 'Clarity') {
      if (value >= 85) return ['Preserve image sharpness and the current copy load', 'Keep important details clear at the intended viewing distance'];
      if (value >= 70) return ['Reduce fine detail near the primary message', 'Check source-art resolution and simplify secondary copy'];
      return ['Simplify the visual field and reduce copy load', 'Check the original artwork for blur and replace unclear source images where needed'];
    }

    if (label === 'Colors') {
      if (value >= 85) return ['Keep the palette focused and the accents intentional', 'Keep a clear light-dark difference between important colors'];
      if (value >= 70) return ['Reduce competing accent colors', 'Use one dominant color field with a limited supporting palette'];
      return ['Reduce competing colors around the main message',
        String(data.colorBlindRisk || '').indexOf('High') !== -1
          ? 'Do not rely on red/green difference alone to communicate meaning'
          : 'Use accent colors selectively to support the main message'];
    }
  ...
  function buildGuidanceHeading(value) {
    ...
    if (value >= 85) return 'What to preserve';
    if (value >= 70) return 'How to refine';
    return 'How to improve';
  }

  function buildSummary(label, value) {
    if (label === 'CTA' && !hasCtaText && Number(value) === 0) return 'No CTA was entered. Call to Action mode assigns 0% here. Enter the CTA as shown, or select Brand Awareness if no action is intended.';
    if (label === 'Readability' && !hasEnteredCopy && Number(value) === 0) return 'No copy was entered, so Readability receives 0%. Enter the ad copy exactly as shown to complete the review.';
    ...
    if (label === 'Colors') {
      ...
      if (colorSignals.uniqueColors) {
        return status + '. The artwork contains about ' + colorSignals.uniqueColors + ' main color groups. Its most common color group covers approximately ' + colorSignals.dominantPercent + '% of the sampled artwork.';
      }
      return status + '. This reflects the number and balance of prominent colors in the artwork.';
    }
    if (label === 'Composition') {
    ...
    if (metric.label === 'CTA' && !hasCtaText && Number(metric.value) === 0) {
      extraNoteHtml =
        '<div class="ac-metric-note">' +
          ...
          '<div class="ac-metric-note-sub">Enter the CTA as shown, or select Brand Awareness if no action is intended.</div>' +
        '</div>';
    }
    if (metric.label === 'Readability' && !hasEnteredCopy && Number(metric.value) === 0) {
      extraNoteHtml =
        '<div class="ac-metric-note">' +
          '<div class="ac-metric-note-warn">Copy missing</div>' +
          '<div class="ac-metric-note-sub">Enter the ad copy exactly as shown to complete the review.</div>' +
        '</div>';
    }
...
function displayActionPlan(data, details) {
    ...
    function guidanceFor(entry) {
        if (entry.key === 'readability') {
            if (!(details && (Number(details.wordCount) > 0 || String(details.headline || '').trim() || String(details.bodyText || details.body || '').trim() || (includeCTA && String(details.cta || '').trim()))) && entry.value === 0) return 'Enter the ad copy exactly as shown to complete the readability review.';
            if (entry.value < 70) return 'Shorten the copy in your artwork. Keep one clear message and remove words that do not support it.';
            return 'Remove unnecessary words while preserving the main message.';
        }
        if (entry.key === 'contrast') {
            ...
            return 'Consider a plain background behind the main message to improve text separation.';
        }
        if (entry.key === 'clarity') {
            ...
            return 'Reduce competing details around the main message.';
        }
        if (entry.key === 'colors') {
            if (entry.value < 70) return 'Simplify the palette and reduce competing accent colors around text.';
            return 'Use accent colors selectively so they do not compete with the main message.';
        }
    ...
    if (!top.length && entries.length) {
        window.acLastActionPlanKeys = [];
        listEl.innerHTML = '<li>No priority changes were identified in this test. Preserve the current strengths of the artwork.</li>';
        return;
    }
    ...
    for (var i = 0; i < top.length; i++) {
        var item = top[i];
        var impact = item.value < 60 ? 'Priority review' : (item.value < 80 ? 'Suggested refinement' : 'Polish');
...
function displayInsights(data, details) {
    ...
    if (!hasDeclaredText && !isPrimaryAction('readability')) {
        ...
        fixes.push('Copy Length: Consider reducing secondary copy to make the main message easier to scan.');
    }
    ...
    if (noAdditionalIssues) {
        fixes.push(actionPlanKeys.length ? 'No additional issues were identified beyond the priority plan above.' : 'No additional issues were identified in this test.');
    }
    ...
    if (includeCTA && ctaState === 'strong') {
        addStrength('cta', data.cta, acGetActionWordBonus(rawCtaText) > 0
          ? 'CTA: The wording is concise and includes a recognized action word.'
          : 'CTA: The wording is concise. Check that it communicates the intended next step.');
    } else if (includeCTA && ctaState === 'ok') {
        addStrength('cta', data.cta, acGetActionWordBonus(rawCtaText) > 0
          ? 'CTA: The wording includes a recognized action word.'
          : 'CTA: The wording is brief. Check that the intended action is clear.');
    }
    ...
    if (noValidatedStrengths) {
        topWorking = ['This review did not identify a clear strength to highlight yet. Focus on the suggested creative improvements above.'];
    }
    var workingHtml = '';
...
if (typeof value !== 'number') {
  ...
  if (isNaN(value)) value = 0;
                ...
                yPos = drawReportList(workingItems, 'Working: ', 80, yPos, 1460, 30, 'This review did not identify a clear strength to highlight yet. Focus on the suggested creative improvements above.');
...
var fixesHeading = isATier ? 'Additional Recommendations' : 'Additional Observations';
		...
		recommendations.push('Keep the main message clear when revising copy, imagery, or layout.');
		recommendations.push('Review the final artwork at a size and distance representative of the intended placement.');
