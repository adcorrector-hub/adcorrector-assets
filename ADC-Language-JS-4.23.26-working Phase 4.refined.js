...
function displayGrade(gradeObj, score, data, details) {
  ...
  var metricDescriptions = {
    ...
    'Clarity': 'How clean and easy the layout appears to process at speed.',
    'Colors': 'How well your colors support legibility and brand recall.',
    'Composition': 'How structured your layout is and how easily the message is seen at a glance.',
  ...
  if (value >= 85) return [
    'The layout appears well structured and easy to process',
    'Maintaining spacing and alignment will help preserve clarity'
  ];

  if (value >= 70) return [
    'Additional spacing may improve separation between elements',
    'Reducing nearby visual competition may help the main message stand out'
  ];

  return [
    'Consider simplifying the layout by reducing competing elements',
    'Text placed near edges may be less visible or at risk of being obscured'
  ];
}
...
if (label === 'Composition') {
  if (value >= 85) return 'The layout appears clean and easy to process at a glance.';
  if (value >= 70) return 'Some structure is present, but elements may compete for attention.';
  return 'The layout may feel cluttered or unbalanced, making the message harder to locate quickly.';
}
...
function displayActionPlan(data, details) {
    ...
    if (modeEl) {
        modeEl.textContent = mode === 'brand' ? 'Brand Awareness' : 'Direct Response';
    }
    ...
    function guidanceFor(entry) {
        ...
        if (entry.key === 'composition') {
            if (entry.value < 70) return 'Move important text away from edges and reduce competing elements around the headline or CTA.';
            return 'Improve spacing and alignment so the hierarchy feels more deliberate.';
        }
        if (entry.key === 'cta') {
...
function displayInsights(data, details) {
    ...
    if (fixes.length < 3 && data.composition < 75 && addressedKey !== 'composition') {
        fixes.push('Mechanical Tension: Increasing white space may improve clarity. Text near edges may be less visible or at risk of being obscured.');
    }
...
if (ocrStatus && ocrStatus.parentNode) {
  ocrStatus.parentNode.removeChild(ocrStatus);
                ...
                if (actionPlanMode) actionPlanMode.textContent = 'Direct Response';
