...
console.log('Readability score:', readabilityScore);
                ...
                if (ctaTrim.length >= 2) {
                  ...
                  // Action word bonus (bonus-only, no penalty for missing)
                  var actionBonus = acGetActionWordBonus(ctaLower);
                  ctaScore += actionBonus;
...
var hasAnchor = hasPhone || hasUrl || hasSocial || hasQr || hasLocationCue;
  ...
  else if (hasQr) ctaScore += 8;     // QR above social
  else if (hasSocial) ctaScore += 6; // social below QR
} else {
