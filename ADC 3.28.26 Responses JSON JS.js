...
// ------------------
// TOP FIXES LOGIC (Deterministic & Physics-Based)
// ------------------
var fixes = [];

// 1. Calculate Dynamic Word Budget based on DOM inputs
...
// 2. PRIORITY 1: Critical Mechanical & Compliance Failures
...
if (data.colorBlindRisk && data.colorBlindRisk.indexOf('High') !== -1) {
    fixes.push('Color Blind Risk: Your color pairing lacks contrast. ~8% of male drivers will struggle to read this.');
}

// 3. PRIORITY 2: Word Budget & Cognitive Load
if (!hasDeclaredText) {
    ...
    fixes.push('Improve text readability by tightening the verb and removing filler words.');
}

// 4. PRIORITY 3: CTA Structure (Direct Response Only)
if (includeCTA && fixes.length < 3) {
    if (ctaState === 'absent') {
        fixes.push('Invisible CTA: Add a concrete anchor (URL, phone, or location cue) to drive direct response.');
    } else if (ctaState === 'weak') {
        fixes.push('Weak Call-to-Action: Your CTA lacks visual dominance. Increase size or use a high-contrast bounding box.');
    }
}

// 5. PRIORITY 4: Structural Bias & Remaining Mechanics
if (hasStrongBias && fixes.length < 3) {
    fixes.push('Imbalanced Hierarchy: High ' + maxMetric.label.toLowerCase() + ' is being dragged down by poor ' + minMetric.label.toLowerCase() + '. Rebalance visual weight.');
}

if (fixes.length < 3 && data.clarity < 75) {
    fixes.push('Layout Clutter: Simplify the design. Remove secondary elements competing with the main message at highway speeds.');
}

if (fixes.length < 3 && data.composition < 75) {
    fixes.push('Mechanical Tension: Increase white space. Pull critical text further away from the mechanical bleed edges.');
}
    ...
    // ------------------
    // WHAT'S WORKING (Pillar-Specific Physics & Design Truths)
    // ------------------
    var working = [];

    // CTA (Direct Mode Only)
    if (includeCTA && ctaState === 'strong') {
        working.push('High-Velocity CTA: A strong action verb and visual anchor ensure the viewer knows exactly what to do.');
    } else if (includeCTA && ctaState === 'ok') {
        working.push('Identifiable CTA: The call to action is present and anchors the layout.');
    }

    // Readability & Clarity
    if (hasDeclaredText && wordsOverBudget <= 0 && data.clarity >= 75) {
        working.push('Crisp & Concise: At ' + actualWordCount + ' words, this layout is perfectly optimized for ' + simSpeed + ' mph comprehension.');
    } else if (data.readability >= 75 && isLegible) {
        working.push('Fast Comprehension: Copy is tight and well-structured for immediate scanning.');
    } else if (data.clarity >= 80) {
        working.push('Edge Definition: High clarity prevents "blur-bleeding" of shapes and text at highway speeds.');
    }

    // Contrast & ADA Compliance
    if (data.adaCompliance && (data.adaCompliance.indexOf('AA') !== -1)) {
        working.push('Maximum Legibility: Contrast ratio (' + data.contrastRatioRaw + ':1) exceeds ADA standards, ensuring visibility in varied lighting.');
    } else if (data.contrast >= 75 && data.readability >= 50) {
        working.push('Distance Integrity: Strong separation between text and background fields protects legibility at a distance.');
    }

    // Composition
    if (data.composition >= 80) {
        working.push('Balanced Breathing Room: Optimal whitespace prevents the design from feeling clustered or cramped.');
    }

    // Colors
    if (data.colors >= 80 && (!data.colorBlindRisk || data.colorBlindRisk === 'Low')) {
        working.push('Color Harmony: The palette supports fast brand recall without creating visual vibration or low-contrast blending.');
    }
