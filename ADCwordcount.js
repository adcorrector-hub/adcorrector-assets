...
function acComputeCtaScore(ctaText) {
  try {
    ...
    // Action verb bonus (conservative list, you can expand later)
    var action = {
      "buy":1,"shop":1,"order":1,"call":1,"text":1,"scan":1,"visit":1,"learn":1,"get":1,"book":1,"reserve":1,
      "join":1,"sign":1,"signup":1,"subscribe":1,"start":1,"try":1,"find":1,"discover":1,"download":1
    };

    // Bonus if any action word appears
    for (var i = 0; i < words.length; i++) {
      var w = words[i].replace(/[^a-z]/g,'');
      if (action[w]) { score += 15; break; }
    }
